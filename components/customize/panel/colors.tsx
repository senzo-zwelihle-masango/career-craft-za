...
import { useResumeUpdate, PanelTip, ResetDefaults, PANEL_TIPS, swatches } from "../_shared"
...
export function ColorsPanel() {
  ...
  const handlePickerChange = useCallback((value: [number, number, number]) => {
    const [r, g, b] = value
    const hex = "#" + [r, g, b].map((x: number) => Math.round(x).toString(16).padStart(2, "0")).join("")
  ...
  return (
    <div className="space-y-4">
      ...
      <ResetDefaults fields={{ accentColor: "#1f2937" }} resume={resume} update={update} />
    </div>
  )
}
