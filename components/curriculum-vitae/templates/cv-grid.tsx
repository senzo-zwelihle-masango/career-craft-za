...
const CurriculumVitaeGrid = ({
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-4 lg:grid-cols-3 xl:grid-cols-4">
      {/* header */}
      <Empty className="aspect-210/297 border border-dashed">
        <EmptyHeader>
          <EmptyMedia variant="icon">
            <HugeiconsIcon icon={CloudIcon} />
          </EmptyMedia>
          <EmptyTitle>
            {cvs.length === 0 ? "No CV's yet" : "Create another CV"}
          </EmptyTitle>
          <EmptyDescription>
            {cvs.length === 0
              ? "Get started by creating a CV."
              : "Add a new CV to your collection."}
          </EmptyDescription>
        </EmptyHeader>
        <EmptyContent>
          <Button
