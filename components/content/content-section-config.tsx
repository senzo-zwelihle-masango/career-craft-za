...
import { FieldDef, EDUCATION_TYPES } from "./content-section-entries-editor"
      ...
      <>
        ...
        {(entry.startDate || entry.endDate) && (
          <p className="mt-1 text-sm text-muted-foreground">
            ...
            {entry.current ? "Present" : (entry.endDate as string)}
          </p>
        )}
      </>
    ),
  ...
  EDUCATION: {
    ...
    renderEntry: (entry: Record<string, unknown>) => (
      <>
        ...
        {entry.type && (
          <span className="mt-0.5 inline-block rounded-full bg-muted px-2 py-0.5 text-xs font-medium capitalize">
            {entry.type as string}
          </span>
        )}
        {(entry.startDate || entry.endDate) && (
          <p className="mt-1 text-sm text-muted-foreground">
            {entry.startDate as string} - {entry.endDate as string}
          </p>
        )}
      </>
    ),
  ...
  CERTIFICATIONS: {
    ...
    renderEntry: (entry: Record<string, unknown>) => (
      <>
        ...
        {entry.issueDate && (
          <p className="mt-1 text-sm text-muted-foreground">
            {entry.issueDate as string}
            {entry.expiryDate ? ` - ${entry.expiryDate as string}` : ""}
          </p>
        )}
      </>
    ),
