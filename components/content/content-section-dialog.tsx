...
import { SECTION_LABELS } from "@/lib/data/editor/types"
import { addSection } from "@/lib/actions/curriculum-vitae"
...
export function ContentSectionDialog({ cvId, existingTypes }: AddSectionDialogProps) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="grid gap-3">
          {available.length === 0 && (
            <p className="text-base text-muted-foreground">All section types have been added.</p>
          )}
          {available.map((type) => {
            ...
            return (
              ...
              >
                {adding === type ? (
                  ...
                  <HugeiconsIcon icon={sectionIcon} className="h-5 w-5 text-muted-foreground" />
                )}
                <div>
                  <p className="font-medium text-base">{SECTION_LABELS[type]}</p>
