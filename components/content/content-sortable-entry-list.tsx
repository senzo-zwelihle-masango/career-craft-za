...
function SortableItem({
  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-start gap-3 rounded-xl border-0 shadow-sm bg-white dark:bg-zinc-900 p-4 hover:shadow-md transition-shadow">
      <button
        {...attributes}
        {...listeners}
    ...
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
      <SortableContext items={entries.map((e) => e.id)} strategy={verticalListSortingStrategy}>
        <div className="space-y-2">
          {entries.map((entry) => (
            <SortableItem key={entry.id} id={entry.id} onDelete={() => handleDelete(entry.id)}>
              {children(entry)}
            </SortableItem>
          ))}
        </div>
