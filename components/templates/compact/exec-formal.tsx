...
export function ExecFormalTemplate({ cv }: { cv: CvWithRelations }) {
    ...
    return (
      <div key={section.id}>
        ...
        {section.type === "EXPERIENCE" && section.experienceEntries.length > 0 && (
          <div style={{ marginTop: 4 * ss }}>
            {section.experienceEntries
              ...
              .sort((a, b) => a.order - b.order)
              .map((entry) => (
                <div key={entry.id} className="break-inside-avoid" style={{ marginBottom: 8 * ss }}>
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-[11px] font-semibold">{entry.role}</p>
                  ...
                  {entry.description && (
                    <div className="prose prose-sm max-w-none mt-1 text-[10px] text-gray-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: entry.description }} />
                  )}
                  {entryStyle !== "paragraph" && entry.bullets.length > 0 && (
                    <ul className="mt-0.5 list-disc pl-4 text-[10px] text-gray-700 leading-relaxed">
            ...
            {section.educationEntries
              ...
              .map((entry) => (
                <div key={entry.id} className="break-inside-avoid" style={{ marginBottom: 6 * ss }}>
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-[11px] font-semibold">{entry.institution}</p>
                      <p className="text-[10px] italic text-gray-700">
                        {entry.degree}
                        {showLocation && entry.location ? ` — ${entry.location}` : ""}
                      </p>
              ...
              .map((entry) => (
                <div key={entry.id} className="break-inside-avoid" style={{ marginBottom: 6 * ss }}>
                  <p className="text-[11px] font-semibold">
                    {entry.name}
                    {entry.link && <span className="text-[10px] text-gray-500 font-normal"> — {entry.link}</span>}
                  </p>
                  {entry.description && (
                    <div className="prose prose-sm max-w-none text-[10px] text-gray-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: entry.description }} />
                  )}
                  {entry.technologies.length > 0 && (
                    <p className="text-[9px] text-gray-500 mt-0.5">{entry.technologies.join(", ")}</p>
                  )}
                </div>
              ))}
          </div>
        ...
        {section.type === "SKILLS" && (
          <div style={{ marginTop: 4 * ss }}>
            {section.skillGroups
              ...
              .sort((a, b) => a.order - b.order)
              .map((group) => (
                <p key={group.id} className="text-[10px] text-gray-800" style={{ marginBottom: 2 * ss }}>
            ...
            {section.certificationEntries
              ...
              .map((entry) => (
                <div key={entry.id} className="break-inside-avoid" style={{ marginBottom: 4 * ss }}>
                  ...
                  {entry.issuer && <p className="text-[10px] text-gray-600">{entry.issuer}</p>}
                  {entry.issueDate && (
              ...
              {section.languageEntries
                ...
                .map((entry) => (
                  <div key={entry.id} className="text-[10px]">
                    ...
                    {entry.proficiency && <span className="text-gray-500"> — {entry.proficiency}</span>}
                  </div>
                ))}
            </div>
          </div>
            ...
            {section.awardEntries
              ...
              .map((entry) => (
                <div key={entry.id} className="break-inside-avoid" style={{ marginBottom: 4 * ss }}>
                  ...
                  {entry.issuer && <p className="text-[10px] text-gray-600">{entry.issuer}</p>}
                  {entry.date && <p className="text-[10px] text-gray-500">{formatDate(entry.date, dateFormat)}</p>}
                  {entry.description && <div className="prose prose-sm max-w-none text-[10px] text-gray-700 mt-0.5" dangerouslySetInnerHTML={{ __html: entry.description }} />}
                </div>
              ))}
          </div>
        )}
        ...
        {section.type === "CUSTOM" && section.content && (
          <div className="prose prose-sm max-w-none text-[10px] text-gray-700 leading-relaxed whitespace-pre-wrap" style={{ marginTop: 4 * ss }} dangerouslySetInnerHTML={{ __html: section.content }} />
        )}
      </div>
    )
  }
  ...
  return (
    ...
    >
      {/* Header */}
      <div style={{ borderBottom: showDividers ? `3px double ${accentColor}` : "none", paddingBottom: 10 * ss, marginBottom: 12 * ss }}>
        <h1 style={{ margin: 0, fontSize: `${1.5 * fs}rem`, fontWeight: 700, color: accentColor, letterSpacing: "0.02em" }}>
          {pd?.fullName || "Your Name"}
        </h1>
        {pd?.jobTitle && (
          <p style={{ margin: "2px 0 0", fontSize: `${0.8125 * fs}rem`, fontStyle: "italic", color: "#374151" }}>
            {pd.jobTitle}
          </p>
        )}
        <div style={{ marginTop: 6 * ss, fontSize: `${0.6875 * fs}rem`, color: "#6B7280", display: "flex", flexWrap: "wrap", gap: "4px 12px" }}>
        ...
        {visibleSections.map((section) => (
          <div key={section.id}>
            ...
            {showDividers && section.type !== "SUMMARY" && (
              <hr style={{ border: "none", borderTop: `1px solid ${accentColor}40`, margin: `${8 * ss}px 0 0` }} />
            )}
          </div>
        ))}
      </div>
