            ...
            .map((entry) => {
              ...
              return (
                <EntryItem
                  ...
                  subtitle={
                    entry.company +
                    (entry.location && showEntryLocation ? ` · ${entry.location}` : "")
                  }
        ...
        <div style={{ marginTop: 4 }}>
          {section.educationEntries
            ...
            .map((entry) => {
              const metaParts: string[] = []
              if (showEntryDates) {
                const sd = entry.startDate ? formatDate(entry.startDate, dateFormat) : ""
                const ed = entry.endDate ? formatDate(entry.endDate, dateFormat) : ""
                if (sd || ed) metaParts.push(sd && ed ? `${sd} — ${ed}` : sd || ed)
              }
              return (
                <EntryItem
                  key={entry.id}
          ...
          {groups
            ...
            .map((group) => (
              <div key={group.id} style={{ marginBottom: 4 }}>
                <span style={{ fontWeight: 600, fontSize: "0.8125em" }}>{group.label}: </span>
      ...
      <div>
        ...
        <div style={{ marginTop: 4 }}>
          {section.languageEntries
            ...
            .sort((a, b) => a.order - b.order)
            .map((entry) => (
              <p key={entry.id} style={{ margin: "2px 0", fontSize: "0.8125em" }}>
                {entry.name}
          ...
          {section.referenceEntries
            ...
            .map((entry) => (
              <div key={entry.id} style={{ marginBottom: 6 }}>
                <h3 style={{ margin: 0, fontSize: "0.875em", fontWeight: 600 }}>{entry.name}</h3>
                {(entry.jobTitle || entry.company) && (
                  <p style={{ margin: "1px 0 0", fontSize: "0.8125em", color: "#374151" }}>
