...
export function MinimalCoverLetter({ data }: { data: CoverLetterTemplateData }) {
  const accent = a(data.accentColor)
  return (
    <div className="bg-white p-8 md:p-10 lg:p-12 text-black min-h-full">
      {data.fullName && <h1 className="text-base font-medium tracking-wide" style={{ color: accent }}>{data.fullName}</h1>}
      {data.professionalTitle && <p className="mt-0.5 text-xs uppercase tracking-widest" style={{ color: `${accent}99` }}>{data.professionalTitle}</p>}
      ...
      {data.date && <div className="mt-4 text-xs" style={{ color: `${accent}99` }}>{data.date}</div>}
