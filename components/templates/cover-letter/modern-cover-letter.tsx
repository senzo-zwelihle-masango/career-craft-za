    ...
    <div className="bg-white text-black min-h-full">
      ...
      <div className="p-8 md:p-10 lg:p-12">
        {(data.fullName || data.professionalTitle) && (
          <div>
            {data.fullName && <h1 className="text-xl font-bold tracking-tight" style={{ color: accent }}>{data.fullName}</h1>}
            {data.professionalTitle && <p className="mt-0.5 text-sm text-gray-500">{data.professionalTitle}</p>}
          </div>
        )}
