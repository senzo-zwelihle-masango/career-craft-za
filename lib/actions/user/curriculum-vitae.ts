...
export async function createCv() {
  ...
  const existingCount = await prisma.curriculumVitae.count({
    where: { userId: session.user.id },
  })
...
export async function updateCv(id: string, json: Record<string, unknown>) {
  ...
  const sectionsData = json.sections as
    | {
  ...
  if (sectionsData?.update) {
    for (const update of sectionsData.update) {
      ...
      if (e.experienceEntries && typeof e.experienceEntries === "object") {
        const ee = e.experienceEntries as {
          create?: unknown[]
          update?: Array<{ where: { id: string }; data: unknown }>
      ...
      if (e.educationEntries && typeof e.educationEntries === "object") {
        const ed = e.educationEntries as {
          create?: unknown[]
          update?: Array<{ where: { id: string }; data: unknown }>
          delete?: Array<{ id: string }>
        }
        if (ed.create) {
          for (const entry of ed.create) {
            const { gpa, field, ...clean } = entry as {
              gpa?: unknown
              field?: unknown
            }
            await prisma.educationEntry.create({
              data: { ...clean, sectionId: where.id } as never,
            })
          }
        }
        if (ed.update) {
          for (const entry of ed.update) {
            const { gpa, field, ...clean } = entry.data as {
              gpa?: unknown
              field?: unknown
            }
        ...
        if (sg.update) {
          for (const group of sg.update) {
            await prisma.skillGroup.update({
              where: { id: group.where.id },
              data: group.data as never,
            })
          }
        }
        if (sg.delete) {
      ...
      if (e.customEntries && typeof e.customEntries === "object") {
        ...
        if (ce.create) {
          for (const entry of ce.create) {
            await prisma.customEntry.create({
              data: {
                ...(entry as Record<string, unknown>),
                sectionId: where.id,
              } as never,
        ...
        if (ce.update) {
          for (const entry of ce.update) {
            await prisma.customEntry.update({
              where: { id: entry.where.id },
              data: entry.data as never,
            })
          }
...
export async function duplicateCv(id: string) {
  ...
  const duplicate = await prisma.curriculumVitae.create({
    data: {
      ...
      elementSpacing: original.elementSpacing,
      marginHorizontal: original.marginHorizontal,
      marginVertical: original.marginVertical,
      ...
      personalDetails: original.personalDetails
        ? {
            create: {
              ...
              links: Array.isArray(original.personalDetails.links)
                ? original.personalDetails.links
                : [],
            },
          }
        : undefined,
...
export async function updateSectionVisibility(
  cvId: string,
  sections: { where: { id: string }; data: { visible: boolean } }[]
) {
  const session = await auth.api.getSession({ headers: await headers() })
  if (!session?.user) return { error: "Unauthorized" }
...
export async function syncCv(id: string, data: Record<string, unknown>) {
  ...
  if (data.personalDetails) {
    const {
      ...
      curriculumVitaeId: _,
      ...pdData
    } = data.personalDetails as Record<string, unknown>
  ...
  if (data.sections) {
    ...
    for (const section of sections) {
      if (!existingSectionIds.has(section.id as string)) continue
      ...
      await prisma.curriculumVitaeSection.update({
        where: { id: section.id as string },
        data: {
          title: sectionData.title as string | undefined,
          content: sectionData.content as string | null | undefined,
          visible: sectionData.visible as boolean | undefined,
          order: sectionData.order as number | undefined,
        },
      })

      if (experienceEntries) {
        const existingEntries = await prisma.experienceEntry.findMany({
          where: { sectionId: section.id as string },
          select: { id: true },
        })
      ...
      if (certificationEntries) {
        ...
        for (const entry of certificationEntries) {
          const { id: eid, sectionId: _, ...ed } = entry
          if (existingIds.has(eid)) {
            await prisma.certificationEntry.update({
              where: { id: eid },
              data: ed,
            })
          }
...
export async function reorderSections(cvId: string, orderedIds: string[]) {
  const session = await auth.api.getSession({ headers: await headers() })
  if (!session?.user) return { error: "Unauthorized" }
...
export async function deleteSection(cvId: string, sectionId: string) {
  ...
  await prisma.curriculumVitaeSection.delete({
    where: { id: sectionId, curriculumVitaeId: cvId },
  })
