...
const jobInclude = {
  ...
  curriculumVitae: { select: { id: true, title: true } },
...
export async function createJobApplication(data: {
  ...
  const job = await prisma.jobApplication.create({
    data: {
      ...
      employmentType: (data.employmentType || null) as never,
      workModel: (data.workModel || null) as never,
      ...
      status: (data.status || "WISHLIST") as never,
...
export async function updateJobApplication(
  ...
  else if (updateData.appliedAt)
    updateData.appliedAt = new Date(updateData.appliedAt as string)

  if (updateData.followUpAt === "") updateData.followUpAt = null
  else if (updateData.followUpAt)
    updateData.followUpAt = new Date(updateData.followUpAt as string)
...
export async function deleteTimelineEvent(id: string) {
  const userId = await getUserId()
  if (!userId) return { error: "Unauthorized" }
  const event = await prisma.jobTimelineEvent.findFirst({
    where: { id, userId },
  })
...
export async function updateOffer(id: string, data: Record<string, unknown>) {
  ...
  if (updateData.deadline)
    updateData.deadline = new Date(updateData.deadline as string)
  if (updateData.respondedAt)
    updateData.respondedAt = new Date(updateData.respondedAt as string)
...
export async function createSavedSearch(data: {
  ...
  const search = await prisma.savedSearch.create({
    data: { ...data, userId, order: count } as never,
  })
...
export async function getJobAnalytics() {
  ...
  const interviewsCompleted = all.reduce(
    (s, j) => s + j.interviews.filter((i) => i.completed).length,
    0
  )
  const offersReceived = all.reduce((s, j) => s + j.offers.length, 0)
  const offersAccepted = all.reduce(
    (s, j) => s + j.offers.filter((o) => o.status === "accepted").length,
    0
  )

  const applied = all.filter(
    (j) => j.status !== "WISHLIST" && j.status !== "REJECTED"
  )
  const responseRate =
    applied.length > 0
      ? Math.round(
  ...
  const interviewToOffer = all.filter(
    (j) => j.interviews.length > 0 && j.offers.length > 0
  ).length
  const interviewConversion =
    all.filter((j) => j.interviews.length > 0).length > 0
      ? Math.round(
          (interviewToOffer /
            all.filter((j) => j.interviews.length > 0).length) *
            100
        )
      : 0
...
export async function getResumeUsageAnalytics() {
  ...
  // Group by resume
  const byResume: Record<
    string,
    { resumeTitle: string; count: number; statuses: Record<string, number> }
  > = {}
  for (const j of jobs) {
    const id = j.curriculumVitaeId!
    if (!byResume[id]) {
      byResume[id] = {
        resumeTitle: j.curriculumVitae?.title || "Untitled",
        count: 0,
        statuses: {},
      }
    }
